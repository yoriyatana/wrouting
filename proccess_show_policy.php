    ...
    if ($count > 0) {
        ...
        while ($row1 = mysql_fetch_array($query1)) {
            if ($elements[$row1['pee_id'] - 1] == true && $row["{$row1['pee_id']}"] == 0) {
                show_policy($prefix, $row1['pee_id'], false);
            }
            if ($elements[$row1['pee_id'] - 1] == false && $row["{$row1['pee_id']}"] == 1) {
                show_policy($prefix, $row1['pee_id'], true);
            }
        }
        ...
        ob_start();
        session_start();
        $sql = "INSERT INTO logs VALUES ('',now(),'User {$_SESSION['wr_uname']} showed command to change routing policy for prefix {$prefix}','{$_SESSION['wr_id']}')";
        mysql_query($sql);
    }
}
?>
